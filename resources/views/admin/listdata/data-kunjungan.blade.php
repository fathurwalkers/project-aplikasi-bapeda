        ...
        <div class="card-body">
            ...
            <div class="table-responsive py-3"> 
              ...
              <table id="table_id" class="display">
                ...
                <tbody>
                    @foreach ($data_kunjungan as $fk)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $fk->tahun }}</td>
                        <td>{{ $fk->mancanegara}}</td>
                        <td>{{ $fk->domestik}}</td>
                        <td>{{ $fk->jumlah}}</td>
                        <td class="d-inline-flex text-center">
                        <a href="{{ url('/admin/data/data-kunjungan-edit') }}/{{$fk->id}}" class="btn btn-primary">Edit</a>
                            <form action="{{ url('/admin/data/data-kunjungan-delete') }}/{{ $fk->id }}" method="POST">
                                @csrf   
